...
std::vector<std::vector<float>> audioInputs;	// variable to save
...
bool setup(BelaContext *context, void *userData) {
	...
	audioInputs.resize(context->audioInChannels);
	try {
		for(auto& c : audioInputs)
			c.resize(numFrames);
...
void render(BelaContext *context, void *userData) {
	...
	for (unsigned int n = 0; n < context->audioFrames; ++n) {
		...
		audioInputs[0][recordingFrames] = currentSample;
		++recordingFrames;
		if(recordingFrames >= audioInputs[0].size()){
			Bela_requestStop();
			return;
		}
	}
...
void cleanup(BelaContext *context, void *userData) {
	for(auto& i : audioInputs)
		i.resize(recordingFrames);
	AudioFileUtilities::write(audioFilename, audioInputs, context->audioSampleRate);
}
